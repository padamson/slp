...
async fn placing_a_tree_shows_canopy_and_trunk_with_no_rotate_handle() -> Result<()> {
    ...
    expect(page.locator("[data-testid='palette-oak-tree']"))
        .to_be_visible()
        .await
    ...
    expect(page.locator("[data-testid='yard'] .furniture-item circle"))
        .to_have_count(2)
        .await
        .context("a tree renders a canopy and a trunk")?;
    expect(page.locator("[data-testid='tree-trunk']"))
        .to_have_count(1)
        .await
    ...
    expect(page.locator("[data-testid='object-inspector']:has-text('⌀ 20 ft')"))
        .to_have_count(1)
        .await
        .context("the inspector shows the tree's canopy diameter")?;
    expect(page.locator("[data-testid='canopy-diameter']"))
        .to_be_visible()
        .await
        .context("a canopy-size input for a tree")?;
    expect(page.locator("[data-testid='trunk-diameter']"))
        .to_be_visible()
        .await
        .context("a trunk-size input for a tree")?;
    expect(page.locator("[data-testid='rotate-handle']"))
        .to_have_count(0)
        .await
        .context("a selected round item shows no rotate handle")?;

    // Bumping the canopy size grows the rendered circle.
    let canopy_input = page.locator("[data-testid='canopy-diameter']");
    canopy_input
        .fill("30", None)
        .await
        .context("set canopy Ø to 30 ft")?;
    expect(page.locator("[data-testid='yard'] .furniture-item circle[r='180']"))
        .to_have_count(1)
        .await
...
async fn dragging_the_canopy_or_trunk_handle_resizes_it() -> Result<()> {
    ...
    let canopy_handle = page.locator("[data-testid='canopy-handle']");
    ...
    // not exactly 30.
    expect(page.locator("[data-testid='object-inspector']:has-text('⌀ 29.9 ft')"))
        .to_have_count(1)
        .await
        .context("the canopy grew toward 30 ft")?;

    // Drag the trunk handle in to (30.5,15) -> radius 0.5 -> Ø 1 ft.
    let trunk_handle = page.locator("[data-testid='trunk-handle']");
...
async fn a_tree_trunk_flags_red_only_on_hardscape() -> Result<()> {
    ...
    place_object(&page, &yard, ppf, "oak-tree", 10.0, 15.0).await?;
    let tree = page.locator("[data-testid='yard'] .furniture-item");
    expect(page.locator("[data-testid='yard'] .furniture-item--trunk-invalid"))
        .to_have_count(0)
        .await
