...
/// course in an area's composition, and other catalog items whose default
/// base/bedding layer points at it.
#[must_use]
pub fn reference_count(plan: &Plan, id: &str) -> usize {
    ...
    let areas = plan
        .shapes
        .iter()
        .map(|s| (&s.material_ref, &s.courses))
        .chain(plan.circles.iter().map(|c| (&c.material_ref, &c.courses)))
        .map(|(material_ref, courses)| {
